...
//    )
    ...
    await MainActor.run {
      ...
      currentTask = Task { @MainActor in
        ...
        // This is not functioning properly and becomes very glitchy when used with the built-in window tiling in macOS.
//        if currentTiling == nextTiling && NSScreen.screens.count > 1 {
//          let pid = pid_t(nextWindow.ownerPid.rawValue)
//          let app = AppAccessibilityElement(pid)
//
//          guard let runningApplication = NSRunningApplication(processIdentifier: pid),
//                let axWindow = try? app.windows().first(where: { $0.id == nextWindow.id }) else {
//            return
//          }
//
...
//          }
//
//          guard let nextScreen = findNextScreen(from: screen, in: tiling, cycling: false) else { return }
//          let nextTiling = nextTiling.revesed
//          let nextVisibleRect = nextScreen.visibleFrame.mainDisplayFlipped
//
//          guard let tilingRect = nextTiling.rect(in: nextVisibleRect, spacing: windowSpacing) else { return }
//
//          var running = true
//          var timeout = 1000
//          var numberOfSuccess = 5
//          axWindow.position = tilingRect.origin
//          while running {
//            if timeout <= 0 {
//              running = false
//              print("timeout")
...
//            axWindow.position = tilingRect.origin
//
//            print(timeout, numberOfSuccess)
//
//            if numberOfSuccess <= 0 {
//              running = false
//            } else if axWindow.frame?.intersects(nextVisibleRect) == true {
//              numberOfSuccess -= 1
...
//          }
//
//          if axWindow.frame?.intersects(nextVisibleRect) == true {
//            print("success")
//          } else {
//            print("failed")
//            return
//          }
//
//          updateStore(isFullScreen: false, isCentered: false, in: nextVisibleRect, for: nextWindow)
//          let match = WindowTilingMenuItemFinder.find(nextTiling, in: menuItems)
//          match?.performAction(.pick)
////          axWindow.frame = tilingRect.insetBy(dx: windowSpacing, dy: windowSpacing)
//          let clickPoint = CGPoint(x: tilingRect.midX, y: tilingRect.midY)
//          NSCursor.moveCursor(to: clickPoint)
//          runningApplication.activate(options: .activateIgnoringOtherApps)
////
////          updateStore(isFullScreen: false, isCentered: false, in: nextVisibleRect, for: nextWindow)
////          WindowStore.shared.index(snapshot.frontmostApplication)
//
////          print("move", nextWindow.ownerName, "to", tilingRect, "on", nextScreen.localizedName)
//

...
extension CGRect {
  ...
  func hasDifference(greaterThan tolerance: CGFloat, comparedTo other: CGRect) -> Bool {
    let xDiff = abs(self.origin.x - round(other.origin.x))
    let yDiff = abs(self.origin.y - round(other.origin.y))
    let widthDiff = abs(self.size.width - round(other.size.width))
    let heightDiff = abs(self.size.height - round(other.size.height))
    let result = xDiff > tolerance ||
                 yDiff > tolerance ||
                 widthDiff > tolerance ||
                 heightDiff > tolerance

    if Self.debug && xDiff > tolerance {
      print("xDiff", xDiff > tolerance)
    }
    if Self.debug && yDiff > tolerance {
      print("yDiff", yDiff > tolerance)
    }
    if Self.debug && widthDiff > tolerance {
      print("widthDiff", widthDiff > tolerance)
    }
    if Self.debug && heightDiff > tolerance {
      print("heightDiff", heightDiff > tolerance)
    }

...
final class TapHeldCoordinator: @unchecked Sendable {
  ...
  private var switchedEvents: [Int64: Int64] = .init()
  ...
  func handlePartialMatchIfApplicable(_ partialMatch: PartialMatch?, machPortEvent: MachPortEvent) -> Bool {
    ...
    case let .event(kind, holdDuration):
                             ...
                             holdDuration: Double)
  {
    ...
    let delay = Int((holdDuration * 1.15) * 1000)
    workItem?.cancel()
                           ...
                           holdDuration: Double)
  {
    if isLeader(newEvent) {
      ...
      case .held:
        ...
        let threshold = CGFloat(Int(holdDuration * 1 * 1000))

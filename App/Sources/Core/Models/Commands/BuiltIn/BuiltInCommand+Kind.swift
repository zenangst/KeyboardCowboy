extension BuiltInCommand {
  enum Kind: Codable, Hashable, Identifiable, Sendable {
    ...
    var id: String {
      switch self {
      case let .macro(macro): macro.id
      case let .userMode(id, action):
        switch action {
        case .enable: "enable-\(id)"
        case .disable: "disable-\(id)"
        case .toggle: "toggle-\(id)"
        }
      case let .commandLine(action): "commandLine-\(action.id)"
      case .repeatLastWorkflow:
    ...
    var userModeId: UserMode.ID {
      switch self {
      case let .macro(action): action.id
      case let .userMode(model, _): model.id
      case let .commandLine(action): action.id
    ...
    init(from decoder: any Decoder) throws {
      ...
      guard let onlyKey = allKeys.popFirst(), allKeys.isEmpty else {
        throw DecodingError.typeMismatch(BuiltInCommand.Kind.self, DecodingError.Context(codingPath: container.codingPath, debugDescription: "Invalid number of keys found, expected one.", underlyingError: nil))
      }
      ...
      case .macro:
        ...
        self = try BuiltInCommand.Kind.macro(action: nestedContainer.decode(MacroAction.self, forKey: BuiltInCommand.Kind.MacroCodingKeys.action))
      case .userMode:
        ...
        self = try BuiltInCommand.Kind.userMode(mode: nestedContainer.decode(UserMode.self, forKey: BuiltInCommand.Kind.UserModeCodingKeys.mode), action: nestedContainer.decode(BuiltInCommand.Kind.Action.self, forKey: BuiltInCommand.Kind.UserModeCodingKeys.action))
      case .commandLine:
        ...
        self = try BuiltInCommand.Kind.commandLine(action: nestedContainer.decode(CommandLineAction.self, forKey: BuiltInCommand.Kind.CommandLineCodingKeys.action))
      case .windowSwitcher:

...
final class KeyboardCowboyConfigurationWorkflowTests: XCTestCase {
  ...
  func testUpdate() {
    ...
    XCTAssertEqual(config.groups.flatMap(\.workflows).map(\.name), ["updated-workflow", "workflow-2", "workflow-3", "workflow-4"])
  }
  ...
  func testReplace() {
    ...
    XCTAssertEqual(config.groups.flatMap(\.workflows).map(\.name), ["workflow-1", "new-workflow", "workflow-3", "workflow-4"])
  }
  ...
  func testAppend() {
    ...
    XCTAssertEqual(config.groups.flatMap(\.workflows).map(\.name), ["workflow-1", "workflow-2", "workflow-3", "workflow-4", "new-workflow"])
  }
  ...
  func testInsert() {
    ...
    XCTAssertEqual(config.groups.flatMap(\.workflows).map(\.name), ["-1", "workflow-1", "workflow-2", "workflow-3", "workflow-4"])
    XCTAssertTrue(config.insert(groupID: "group-1", workflow: Workflow(name: "0"), at: 0))
    XCTAssertEqual(config.groups.flatMap(\.workflows).map(\.name), ["0", "-1", "workflow-1", "workflow-2", "workflow-3", "workflow-4"])
    XCTAssertTrue(config.insert(groupID: "group-1", workflow: Workflow(name: "100"), at: 100))
    XCTAssertEqual(config.groups.flatMap(\.workflows).map(\.name), ["0", "-1", "workflow-1", "workflow-2", "workflow-3", "workflow-4", "100"])
